...
function ChemistryAlchemy() {
    ...
    const handleCombine = async () => {
        ...
        let reactionResult = reactions[key] || "❌ No reaction";
        if (reactionResult === "❌ No reaction"){
            try {
                let data;
                try {
                    const response = await fetch("http://localhost:5001/find_reaction", {
                        method: "POST",
                    ...
                    console.log(data);
                    if (data.data === "no") { reactionResult = "❌ No reaction";}
                    else {reactionResult = data.data;} 
                } catch (error) {
                    console.warn("Could not connect to server, using mock data", error);
                }
            } catch (error) {
